...
def process_files_concurrently(uploaded_files, openai_client):
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for i, uploaded_file in enumerate(uploaded_files):
            st.info(f"Submitting file {i+1}/{len(uploaded_files)}: {uploaded_file.name} for processing")
            if uploaded_file.type in ["video/quicktime", "video/mp4"]:
                ...
                futures.append(executor.submit(openai_client.transcribe_audio, uploaded_file))
