        ...
        if uploaded_file.type in ["video/quicktime", "video/mp4"]:
            ...
            if audio_file_path:
                with open(audio_file_path, "rb") as f:
                    transcriptions.append(openai_client.transcribe_audio(f))
        elif uploaded_file.type == "audio/mpeg":
            transcriptions.append(openai_client.transcribe_audio(uploaded_file))
        elif uploaded_file.type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
            transcriptions.append(read_docx(uploaded_file))
        elif uploaded_file.type == "text/plain":
            transcriptions.append(read_txt(uploaded_file))
        elif uploaded_file.type == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet":
            transcriptions.append(read_excel(uploaded_file))
        elif uploaded_file.type == "application/pdf":
            transcriptions.append(read_pdf(uploaded_file))
        elif uploaded_file.type == "application/vnd.openxmlformats-officedocument.presentationml.presentation":
            transcriptions.append(read_pptx(uploaded_file))
        elif uploaded_file.type in ["image/jpeg", "image/png"]:
            image = Image.open(uploaded_file)
            base64_image = encode_image(image)
            transcriptions.append(openai_client.transcribe_image(base64_image))
